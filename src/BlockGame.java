...
public class BlockGame extends JPanel
		implements Runnable, MouseListener, MouseMotionListener, KeyListener, FocusListener {
	...
	public BlockGame() {
		...
		int maxDepth = Integer
				.parseInt(JOptionPane.showInputDialog("Choose the maximum number of subdivisions allowed", 6));
		...
		playerCount = 1 + JOptionPane.showOptionDialog(null, "How many players?", "Playercount",
		...
		if (playerCount == 1) {
			JOptionPane.showOptionDialog(null, this.p1goal.description(), "Player 1 - Goal", JOptionPane.DEFAULT_OPTION,
			...
			JOptionPane.showOptionDialog(null, this.p1goal.description(), "Player 1 - Goal", JOptionPane.DEFAULT_OPTION,
					JOptionPane.INFORMATION_MESSAGE, null, new String[] { "OK" }, "OK");
			JOptionPane.showOptionDialog(null, this.p2goal.description(), "Player 2 - Goal", JOptionPane.DEFAULT_OPTION,
					JOptionPane.INFORMATION_MESSAGE, null, new String[] { "OK" }, "OK");
			p1ScoreLabel.setForeground(GameColors.BLOCK_COLORS[p1TargetColor]);
	...
	private void updateScores() {
		...
		p1ScoreLabel.setText(
				"Player 1's " + (p1goal instanceof BlobGoal ? "area" : "perimeter ") + " score: " + this.p1Score);
		p2ScoreLabel.setText(
				"Player 2's " + (p2goal instanceof BlobGoal ? "area" : "perimeter ") + " score: " + this.p2Score);
	}
	...
	private void makeMove() {
		...
		if (turnsLeft == 0) { // == 0 is better than <= 0 because it lets them play on after the game "ends"
								// if they want
			if (playerCount == 1) {
